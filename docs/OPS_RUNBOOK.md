...
| 4 app smoke | `getKgContext` (the routes' entry point) with `KG_ENABLED=true` → GROUNDED, bundle id issued, 3.9 s cold / warm thereafter, inside the 10 s guard. **Dev-server smoke 2026-09-26** (after `.env.local` was switched): `next dev` on 3091, one `POST /api/render` on the dev demo villa's living room → the log shows `[api/render] KG context injected (bundle=bundle:1790397949572:hqcpxc)` with the full Mudon grounding block, no `[lib/kg] KG retrieval failed` line, HTTP 200 in 17.5 s (the render model, not the KG); render row `e572ad4d` carries the `kg_bundle_id`. The style choice locked for the test was removed afterwards; the dev demo is as it was plus one render row |
