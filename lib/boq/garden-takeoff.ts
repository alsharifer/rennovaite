...
const SECTION: Record<string, PomiSection> = {
  ...
  "garden.pcc_base": "External Works",
  "garden.paving_install": "External Works",
  "garden.tile_supply": "External Works",
  "garden.grass_supply": "External Works",
  "garden.grass_install": "External Works",
  "garden.counter_bbq": "Landscape Structures",
  "garden.counter_bar": "Landscape Structures",
  "garden.bench_run": "Landscape Structures",
  "garden.planter_bench_run": "Landscape Structures",
  "garden.planter_box": "Landscape Structures",
  "garden.wall_feature": "Landscape Structures",
  "garden.pergola": "Landscape Structures",
  "garden.bbq_grill": "Landscape Structures",
  "garden.irrigation": "Irrigation",
  "garden.light_cabling": "External Lighting",
  "garden.light_fitting": "External Lighting",
  "garden.boundary_light": "External Lighting",
};
...
/**
 * Irrigation reference driver, in "planting m² + planter lm" units.
 *
 * Villa 94: 7.8 lm of planter+bench and no separately recorded planting area.
 * That is a thin basis for scaling anything, which is exactly why every scaled
 * irrigation line is flagged `qty_derived` and the scale is clamped — a lump
 * stretched five times over is not a lump any more, it is a guess wearing one.
 */
const IRRIGATION_REFERENCE_DRIVER = 7.8;
const IRRIGATION_SCALE_MIN = 0.5;
const IRRIGATION_SCALE_MAX = 2;
...
export function computeGardenTakeoff(input: GardenTakeoffInput): GardenTakeoff {
  ...
  if (pavedAreaM2 > 0) {
    items.push(
      item("GL-04", "garden.pcc_base", pavedAreaM2, `paved zone area = ${pavedAreaM2} m² (${VILLA94_PCC_VS_PAVING_NOTE})`),
      item("GL-05", "garden.paving_install", pavedAreaM2, `paved zone area = ${pavedAreaM2} m²`),
      ...
      item("GL-06", "garden.tile_supply", pavedAreaM2, `paved zone area = ${pavedAreaM2} m²; tile is bought per m² PURCHASED, which site setting-out usually reduces`, {
  ...
  const runTotal = (kind: GardenRun["kind"], variant?: GardenRun["variant"]) =>
    round2(
      runs
        .filter((r) => r.kind === kind && (variant === undefined || (r.variant ?? "bar") === variant))
  ...
  const barLm = runTotal("counter_run", "bar");
  if (barLm > 0) {
    items.push(item("GL-11b", "garden.counter_bar", barLm, `Σ bar counter length = ${barLm} lm`));
  }
  ...
  if (irrigationDriver > 0) {
    const rawScale = irrigationDriver / IRRIGATION_REFERENCE_DRIVER;
    const scale =
      Math.round(Math.min(IRRIGATION_SCALE_MAX, Math.max(IRRIGATION_SCALE_MIN, rawScale)) * 100) / 100;
    const clamped = Math.abs(scale - rawScale) > 1e-9;
    items.push(
      item("GL-15", "garden.irrigation", scale, `lump × ${scale} — driver ${irrigationDriver} (planting ${plantingAreaM2} m² + planter ${planterLm} lm) against a reference garden's ${IRRIGATION_REFERENCE_DRIVER}${clamped ? `; raw ${round2(rawScale)} clamped to ${IRRIGATION_SCALE_MIN}–${IRRIGATION_SCALE_MAX}` : ""}`, {
        // The RATE is a transacted actual; the QUANTITY is inferred from one
        // reference project. Those are different claims and the line makes both.
        qty_derived: true,
      }),
    );
  }
