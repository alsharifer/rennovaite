...
export interface ComponentOwner {
   ...
   */
  conflicts: { mechanism: PricingMechanism; key: string }[];
...
export const COMPONENT_OWNERS: ComponentOwner[] = [
  ...
  },
  {
    ...
    conflicts: [
      { mechanism: "actual", key: "floor.porcelain_material" },
      { mechanism: "section_rule", key: "join.vanity" },
    ],
    source_cell: "G19",
    note:
  ...
  },
  {
    ...
    conflicts: [{ mechanism: "section_rule", key: "stairs.renovation" }],
    source_cell: "G15",
    note:
 ...
 */
function lineMatches(line: DedupeLine, key: string): boolean {
  if (line.item_key && line.item_key === key) return true;
  const rid = line.rule_id ?? "";
  return rid.startsWith("P2/overlay/") && rid.slice("P2/overlay/".length) === key;
}
