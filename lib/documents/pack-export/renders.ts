// =============================================================================
// lib/documents/pack-export/renders.ts — render every view of a garden, in the
// order that keeps them consistent (garden pilot G5c; moved here from
// scripts/lib/garden-render-run.ts in T5 so the in-app export and the CLI run
// the SAME anchor-first + consistency sequence).
//
...
export async function renderAllViews(opts: {
   ...
   */
  anchorImage?: boolean;
  /** T5: answer every view from cache / reuse only — a view that needs rendering is reported "not_cached". */
  cacheOnly?: boolean;
  ...
  const one = async (camera: CameraRow, view: "day" | "evening", anchorRenderId: string | null): Promise<RenderedView> => {
    const t0 = Date.now();
    const r = await post("/api/render/scene", { project_id: projectId, camera_id: camera.id, view, ...(anchorRenderId && view === "day" ? { anchor_render_id: anchorRenderId } : {}), ...(opts.cacheOnly ? { cache_only: true } : {}) });
